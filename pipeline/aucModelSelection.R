select.auc.model = function(locale.input, fetched.variable) {
    preliminary.selected.model.number = auc.means[locale.input$selected.model.number, "Model"]
    ...
    if (select.with.number) {
        ...
        selected.model.number = auc.means[which(auc.headers[,"Headers"] == preliminary.selected.model.header), "Model"]
    }
