...
library(pROC)
 ...
 iterations.route = file.path(base.route, "Pipeline_Output")
 iteration.dir=file.path(iterations.route, iteration.dir)
...
auc.names = c("Iteration", "Model", "ModNames", "AUC")
            ...
            if(num.preds>2){
                ...
                if (length(unique(class.subset)) > 1) {
                    ...
                    if(any( score.data > 0)){
                        ...
                        auc.temp.matrix=matrix(nrow=1, ncol=length(auc.names), data=(c(iteration.locale.index, model.locale.index, model.name, auc.value)))
                        auc.data.table =rbind(auc.data.table , auc.temp.matrix)
                    }
                }
               
            }
            combined.model.prediction.table = cbind(rep(iteration.locale.index, num.preds), rep(model.locale.index, num.preds), rep(model.name, num.preds), prediction.table$Response, prediction.table)
