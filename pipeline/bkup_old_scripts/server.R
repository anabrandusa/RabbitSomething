    ...
    output$medianAuc <- renderPlot({
        ...
        for (model.subset in models.for.step) {
            ...
            for (model.name in model.subset) {
                ...
                message("Using subtable ", selected.subtable)
        ...
        message("Using AUC data ", auc.for.subsets)
        boxplot(auc.for.subsets, ylim=c(0,1), names=names(models.for.step), col=box.col[1:length(models.for.step)])
    })
